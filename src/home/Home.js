...
class Home extends Component {
  ...
  handleClose = (newValue) => {
    ...
    if (newValue) {
      const project_selected = this.state.projects.filter(
        (proj) => proj.id === newValue
      );
  ...
  registerProjects(user) {
    ...
    this.unsubscribeProjects = this.userRef.doc(user_id).onSnapshot((doc) => {
      ...
      if (doc && doc.exists && doc.data().list_of_projects) {
        ...
        if (projRefs) {
          projRefs.forEach((projRef) => {
            projRef.get().then((doc) => {
              ...
              if (doc && doc.exists && doc.data().list_of_requests) {
  ...
  makeDataRequest = (dataset) => {
    ...
    db.collection("requests")
      ...
      .then(function (requestRef) {
        ...
        projectRef.update({
          list_of_requests:
            firebase.firestore.FieldValue.arrayUnion(requestRef),
        });
        ...
        {this.props.user ? (
          <div className="home-container">
            <div className="project-display">
              ...
              <h2>
                {this.state.project_selected.name.length > 0
                  ? this.state.project_selected.name
                  : "No project selected"}
              </h2>
            </div>
            ...
            <div className="flexGrow">
              {this.state.datasets.map((dataset) => (
                <HomeDatasetRow
                  ...
                  datasetsRequested={
                    this.state.datasetsRequested[this.state.project_selected.id]
                  }
                />
              ))}
            </div>
